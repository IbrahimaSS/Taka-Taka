...
function parseAdminPath(pathname) {
  if (!pathname.startsWith(ADMIN_BASE)) return { segments: [], first: '' };
...
export default function Header({ currentDate, onMenuToggle, onSidebarToggle, sidebarCollapsed, showToast }) {
  ...
  const { segments, first } = useMemo(() => parseAdminPath(location.pathname), [location.pathname]);
  const pageTitle = TITLES[first] || TITLES[''];

  const breadcrumbs = useMemo(() => {
    const crumbs = [{ label: platform.name || 'Admin', to: ADMIN_BASE }];
