...
const RealTimeTracking = ({
  ...
  const simulateTripMovement = useCallback(() => {
    if (progress >= 100 || isTripEnded) {
      clearInterval(tripSimulationInterval.current);

      if (progress >= 100 && !isTripEnded) {
        setIsTripEnded(true);
        showToast('🎉 Vous êtes arrivé à destination !', 'success');
    ...
    // Calculer la nouvelle position du chauffeur
    const start = tripData.departure.coords;
    const end = tripData.destination.coords;
    const progressRatio = newProgress / 100;

    const lat = start[0] + (end[0] - start[0]) * progressRatio;
    const lng = start[1] + (end[1] - start[1]) * progressRatio;

    setDriverPosition([lat, lng]);

    // Mettre à jour la vitesse aléatoire
    setSpeed(Math.floor(Math.random() * 20) + 30);

    // Mettre à jour la distance parcourue
    tripData.trip.traveledDistance = (tripData.trip.totalDistance * progressRatio).toFixed(1);
    tripData.trip.elapsedMinutes = Math.floor(tripData.trip.totalDuration * progressRatio);
    tripData.trip.remainingMinutes = Math.max(0, tripData.trip.totalDuration - tripData.trip.elapsedMinutes);

    // Calculer l'heure d'arrivée estimée
    const now = new Date();
    now.setMinutes(now.getMinutes() + tripData.trip.remainingMinutes);
    setEstimatedArrival(`${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`);
    ...
    // Notification à 90%
    if (newProgress >= 90 && newProgress < 92) {
      showToast('🛑 Vous approchez de votre destination', 'info');
    }
  }, [progress, isTripEnded, tripData, onEndTrip]);
  ...
  // Réinitialiser le trajet (pour démo)
  const handleResetTrip = () => {
    clearInterval(tripSimulationInterval.current);
    setProgress(0);
    setDriverPosition(tripData.departure.coords);
    setSpeed(0);
    setIsTripEnded(false);

    // Redémarrer la simulation après 1 seconde
    setTimeout(() => {
      tripSimulationInterval.current = setInterval(simulateTripMovement, 2000);
    }, 1000);
  ...
  }, [updateTime, simulateTripMovement]);
