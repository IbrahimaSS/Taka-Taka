...
import {
    ...
    Target, Navigation as NavigationIcon, Clock as ClockIcon,
    TrendingUp, Award, Zap, Battery, Wifi, Signal, BatteryCharging,
    Compass, Radio, Shield as ShieldIcon
} from 'lucide-react';
...
// MapController is now imported from a shared component



// Sous-composant pour la carte
const TrackingMap = ({
    driverLocation,
    acceptedTrips,
    activePickupCoords,
...
// Sous-composant pour la liste des passagers
const PassengerList = ({
    acceptedTrips,
    currentPickupTripId,
    selectPickupTrip,
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
                                        ...
                                        <div>
                                            <Button
                                                ...
                                                onClick={() => selectPickupTrip(trip.id)}
                                                className='w-full h-11'
                                            >
                                                {currentPickupTripId === trip.id ? 'EN COURS' : 'REJOINDRE'}
                                            </Button>
                                        </div>
                                    </div>
                                </motion.div>
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3  gap-4 mb-6">
                ...
                {tripStep === 'ready_to_start' && (
                    <Button
                        variant="success"
                        size="large"
                        icon={FlagIcon}
                        onClick={() => handleConfirm(startCourse)}
                        fullWidth
    ...
    useEffect(() => {
        ...
        if (tripStep === 'in_progress' || tripStep === 'to_pickup') {
            const interval = setInterval(() => {
                setSpeed(prev => {
                    const change = Math.random() * 10 - 5; // Variation de -5 à +5
