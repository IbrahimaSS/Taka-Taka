                ...
                <div className="lg:col-span-2 surface dark:bg-gray-800/80 rounded-2xl p-8 shadow-xl border border-white/20 dark:border-gray-700/50">
                    ...
                    <div className="flex items-center space-x-6 mb-8">
                        <div className="relative">
                            <div className="w-32 h-32 rounded-full bg-gradient-to-br from-primary-100 to-secondary-200 dark:from-primary-900/40 dark:to-secondary-900/40 flex items-center justify-center shadow-lg overflow-hidden border-4 border-white dark:border-gray-700">
                                {profileData.avatar ? (
                                    <img src={profileData.avatar} alt="Profile" className="w-full h-full object-cover" />
                                ) : (
                                    <User className="w-16 h-16 text-primary-700 dark:text-primary-300" />
                                )}
                            </div>
                            <input type="file" ref={fileInputRef} onChange={handleImageUpload} accept="image/*" className="hidden" />
                            <button
                                onClick={() => fileInputRef.current?.click()}
                                className="absolute bottom-2 right-2 w-10 h-10 bg-primary-600 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-primary-700 transition-colors border-2 border-white dark:border-gray-800"
                            >
                            ...
                            <div className="flex items-center space-x-4 mb-2">
                                {isEditing ? (
                                    <input
                                        ...
                                        className="text-3xl font-bold text-gray-900 dark:text-gray-100 bg-transparent border-b-2 border-primary-500 focus:outline-none"
                                    />
                                ) : (
                    ...
                    <div className="space-y-8">
                        <Card className="bg-transparent border-none shadow-none p-0">
                            ...
                            <CardContent className="px-0">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="block text-gray-700 dark:text-gray-300 font-medium mb-2 text-sm">Nom complet</label>
                                        <input
                                            disabled={!isEditing}
                                            type="text"
                                            value={profileData.name}
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-4 pt-8 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div className="flex space-x-3">
                                {isEditing ? (
                                    <>
                                        <Button variant="secondary" onClick={() => { setProfileData({ ...profile }); setIsEditing(false); }}>Annuler</Button>
